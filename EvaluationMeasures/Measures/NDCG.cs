...
namespace MyMediaLite.Eval.Measures
{
	...
	/// </remarks>
	public static class NDCG<T>
	{
	    public static double Compute(IList<T> ranked_items, ICollection<T> correct_items, int n)
	    {
	        return Compute(ranked_items.ToList().GetRange(0, n), correct_items.ToList().GetRange(0, n));
	    }
		...
		public static double Compute(IList<T> ranked_items, ICollection<T> correct_items)
		{
			double dcg   = 0;
